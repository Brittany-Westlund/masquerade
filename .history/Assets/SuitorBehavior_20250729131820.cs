...
public class SuitorBehavior : MonoBehaviour
{
    ...
    [Header("Animation")]
    public Animator animator;                 // Assign your flamingo's Animator
    ...
    public System.Collections.IEnumerator Scoot()
    {
        ...
        // Force idle-only visuals while moving
        FlamingoSuitor visualState = GetComponent<FlamingoSuitor>();
        if (visualState != null)
        {
            visualState.SetToIdleOnly();
        }
        ...
        {
            Vector3 scale = transform.localScale;
            scale.x = Mathf.Abs(scale.x) * Mathf.Sign(moveDistance); // Ensure correct facing
            transform.localScale = scale;
        }
        ...
        // Start return timer *after* scooting
