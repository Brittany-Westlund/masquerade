...
public class PetalSpawner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = petals.Count - 1; i >= 0; i--)
        {
            ...
            if (!data.hasLanded && elapsed < data.fallDuration)
            {
                // Fall and flutter
                Vector3 pos = data.obj.transform.position;
                pos.y -= fallSpeed * Time.deltaTime;
                float drift = Mathf.Sin(elapsed * flutterFrequency) * flutterAmplitude;
            ...
            // Start fade if lifetime exceeded
            if (!data.isFading && elapsed > petalLifetime)
            {
                data.isFading = true;
                data.fadeStartTime = now;
            }

            // Fade out
            if (data.isFading && data.spriteRenderer)
            {
                float fadeElapsed = now - data.fadeStartTime;
                float fadeDuration = 1f;
                float alpha = Mathf.Lerp(1f, 0f, fadeElapsed / fadeDuration);

                Color c = data.spriteRenderer.color;
                c.a = alpha;
    ...
    void SpawnPetals()
    {
        for (int i = 0; i < petalsPerWave; i++)
        {
            ...
            SpriteRenderer sr = petal.GetComponent<SpriteRenderer>();
